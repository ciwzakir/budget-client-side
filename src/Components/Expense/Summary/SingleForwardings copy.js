...
const SingleForwardings = ({summary}) => {
  const { item_supplier, total_exp, total_tds, total_vds, total_paid } = useContext(ForwardingsContext);
    ...
    <div className="my-11 w-2/3 ml-24 ">
      <div className="cheque px-12 text-justify" ref={componentRef}>
        ...
        <p className="pr-7">
          ...
          <span className="supplier-info underlinesdotted">
            {total_exp}
          </span>
        ...
        {summary?.filter((nested) => (
          <ForwardingsContext.Provider value={nested}>
              <DeepNestedMap  ></DeepNestedMap>
      ...
      <p className="text-justify">Please, acknowledge the receipt of the cheque by attaching a revenue stamp of tk.10. If you have any query regarding this payment, do not hesitate to contact us. </p>
